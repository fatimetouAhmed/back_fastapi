...
def get_etudiant(photo: str):
    ...
    if etudiant:
        print("etudiant",etudiant["_id"])
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        subquery = db.etudiermat.aggregate([
          {
               "$lookup": {
                    "from": "etudiant",
                    "localField": "id_etu",
          ...
          {
               "$match": {
                    "etudiermat2.id_etu": etudiant["_id"]
               }
          },
          {
               "$project": {
        ...
        subquery_ids = [item["id_mat"] for item in subquery]
        print(subquery_ids)
        print("date", now)
        exams = db.examun.find( { "heure_deb": {"$lte": now},
                                 "heure_fin": {"$gte": now}, 
                                 "id_mat": {"$in": subquery_ids} }, { "_id": 1 } )
        exam = [item["_id"] for item in exams]
        print("exam", exam)
        if not exam:
         ...
         return "l etudiant n est pas trouver"
